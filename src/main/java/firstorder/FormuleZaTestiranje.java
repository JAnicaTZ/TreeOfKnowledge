 ...
 * …) that return syntactically valid test strings using the calculator's Unicode operator
 * constants.
 *
 * <p>Used by {@link Calc} to:
 *
 * <ul>
 *   <li>Populate the input field with representative examples,
 *   <li>Demonstrate supported syntax (quantifiers, predicates, connectives),
 *   <li>Offer regression tests during development (parser + engine pipeline).
 * </ul>
 *
 * <p>This class contains no logic-engine implementation; it is intentionally kept as a “test
 * catalogue” to keep the UI code clean and to make demonstrations reproducible.
 */
...
public class FormuleZaTestiranje {
  ...
  public static String formula4() {
    return ((new Character(Calc.POSTOJI_CHAR)).toString()
                ...
                + (new Character(Calc.y_CHAR)).toString()
                + (new Character(Calc.POVLACI_CHAR)).toString()
                + new Character(Calc.LIJEVA_ZAGRADA))
            .toString()
        + (new Character(Calc.ZASVAKI_CHAR)).toString()
        + (new Character(Calc.x_CHAR)).toString()
        + (new Character(Calc.POSTOJI_CHAR)).toString()
