 ...
 *
 * <p>This class is a key transformation step in the engine: it pushes negation inward and
 * normalizes the formula structure. In particular, it supports:
 *
 * <ul>
 *   <li>Double-negation elimination (¬¬A → A),
 *   <li>De Morgan transformations (¬(A ∧ B) → (¬A ∨ ¬B), ¬(A ∨ B) → (¬A ∧ ¬B)),
 *   <li>Quantifier flipping under negation by transforming the quantifier prefix (∀ ↔ ∃) via {@link
 *       #suprotanPrefix(String)}.
 * </ul>
 *
...
public class Negacija extends Formula {
  ...
  public Formula glavniTestLogikePrvogReda() throws Clear {
    ...
    if (podStablo instanceof Negacija) {
      ((Negacija) podStablo).podStablo.prefix +=
          prefix + suprotanPrefix(((Negacija) podStablo).prefix);
      return ((Negacija) podStablo).podStablo.glavniTestLogikePrvogReda();
    }
    if (podStablo
        instanceof Konjukcija) { // de Morgan() - !! Konjukcija & Disjunkcija imaju prefix?!
      BinarnaFormula podStablo = (BinarnaFormula) this.podStablo;
      podStablo =
          new Disjunkcija(
              new Negacija(prefix + suprotanPrefix(podStablo.prefix), podStablo.lijevoPodStablo),
  ...
  public static String suprotanPrefix(String prefix) {
    ...
    for (int i = 0; i < prefix.length(); i++) {
      suprotanPrefix +=
          (prefix.charAt(i) == Calc.ZASVAKI_CHAR) ? Calc.POSTOJI_CHAR : Calc.ZASVAKI_CHAR;
