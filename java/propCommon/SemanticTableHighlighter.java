...
public class SemanticTableHighlighter {
  public static List koristeneVarijable;
  public static String[] semantickaTablica;
  public static String razmak = "    ";

  // public static void osvijetliRjesenja(List koristeneVarijable, List
  // disjunktivnaForma) {
  ...
  // JButton gumbInterpretacije = ((JButton)
  // Calc.interpretacijePanel.getComponent(i + 2));
  ...
  // ((JButton) Calc.interpretacijePanel.getComponent(i +
  ...
  public static void osvijetliRjesenja(
      List<Character> koristeneVarijable,
      List<List<AtomicFormula>> disjunktivnaForma, JPanel panel) {
    SemanticTableHighlighter.koristeneVarijable = koristeneVarijable;
    String varijable = "";
    for (int i = 0; i < koristeneVarijable.size(); i++) {
      varijable += razmak + ((Character) koristeneVarijable.get(i)).charValue();
    }
    // JPanel panel = Calc.interpretacijePanel;
    ...
    for (int i = 0; i < semantickaTablica.length; i++) {
      ...
      while (++k <= disjunktivnaForma.size())
        if (SemanticTableHighlighter.zadovoljavaInterpretaciju(
  ...
  /**
   * @return true iff the binary assignment is compatible with the given
   *         conjunctive clause.
   *         Compatibility means: all required literals match the assignment, and
   *         no literal contradicts
   *         it.
   */
  public static boolean zadovoljavaInterpretaciju(String nuleJedinice, List konjukt) {
    int i = 0;
    while (i < koristeneVarijable.size()) {
      AtomicFormula a = new AtomicFormula(((Character) koristeneVarijable.get(i)).charValue());
