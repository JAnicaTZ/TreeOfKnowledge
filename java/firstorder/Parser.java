 ...
 *
 * <ul>
 *   <li>Supports a compact prefix notation for quantifiers (e.g., ∀x∃y...) used by the engine.
 *   <li>Tracks variables that actually occur in the formula (stored in {@code
 *       mapKoristeneVarijable}).
 *   <li>Implements a recursive-descent style grammar: factors, unary negation, binary operators.
 *   <li>Builds the internal tree structure used by the normalization and testing pipeline.
 * </ul>
 *
 * <p>The parser is intentionally strict and throws small, domain-specific exceptions (e.g. wrong
 * input / missing bracket / backspace / clear) to keep the UI responsive and to allow immediate
...
public class Parser {
  ...
  public static Formula term(String prefix)
      throws PogresanUnos, ZatvoriZagradu, BackSpace, Exception {
    int k = i;
    ...
    while ((i < duljinaFormule) && Calc.binarniVeznici.contains(new Character(formula.charAt(i)))) {
      ...
      if (i + 1 >= duljinaFormule)
        throw new PogresanUnos();
      switch (formula.charAt(i++)) {
        ...
        case Calc.POVLACI_CHAR:
          ...
          ((Negacija) ((BinarnaFormula) binarniVeznik).lijevoPodStablo).podStablo.prefix = ((Negacija) ((BinarnaFormula) binarniVeznik).lijevoPodStablo).podStablo.prefix
        ...
        case Calc.AKKO_CHAR: {
          ...
          Formula imp1 = new Disjunkcija(
              new Negacija(prefix, (Formula) prviFaktor.clone()),
              (Formula) drugiFaktor.clone()); // A -> B
          Formula imp2 = new Disjunkcija(
              new Negacija(prefix, (Formula) drugiFaktor.clone()),
  ...
  public static Formula faktor(String prefix)
      throws PogresanUnos, ZatvoriZagradu, BackSpace, Exception {
    if (i >= duljinaFormule
        || !(Calc.kvantifikatori.contains(new Character(formula.charAt(i)))
            || Calc.propozicVarijable.contains(new Character(formula.charAt(i)))))
      throw new PogresanUnos();
    while ((i < duljinaFormule)
        && (Calc.kvantifikatori.contains(new Character(formula.charAt(i)))
            || Calc.varijable.contains(new Character(formula.charAt(i))))) {
      if (prefix.length() >= 4 || !Calc.kvantifikatori.contains(new Character(formula.charAt(i))))
        throw new PogresanUnos(); // prevelikPrefix();
      prefix += (new Character(formula.charAt(i++))).toString();
      if (i >= duljinaFormule || !Calc.varijable.contains(new Character(formula.charAt(i))))
        throw new PogresanUnos();
      prefix += (new Character(formula.charAt(i++))).toString();
    }
    if (i >= duljinaFormule)
    ...
    switch (formula.charAt(i)) {
      ...
      case Calc.Q_CHAR:
        ...
        if (i >= duljinaFormule)
          throw new PogresanUnos();
        if (!Calc.varijable.contains(new Character(formula.charAt(i))))
          throw new BackSpace();
        while ((i < duljinaFormule) && Calc.varijable.contains(new Character(formula.charAt(i)))) {
          if (i >= duljinaFormule
              || atomarnaFormula.length() >= 3
              || !Calc.varijable.contains(new Character(formula.charAt(i))))
            throw new PogresanUnos(); // previseVarijabli();
          atomarnaFormula += (new Character(formula.charAt(i++))).toString();
        }
        if (!mapKoristeneVarijable.containsKey(
            (new Character(atomarnaFormula.charAt(0))).toString())) {
          if ((atomarnaFormula.length() - 1) == 2
              && mapKoristeneVarijable.containsValue(new Integer(2)))
            throw new BackSpace(); // !! neDrzimo();
          mapKoristeneVarijable.put(
              (new Character(atomarnaFormula.charAt(0)).toString()),
              new Integer(atomarnaFormula.length() - 1));
        } else { // mapKoristeneVarijable.containsKey((new
          // Character(atomarnaFormula.charAt(0))).toString())
          if (!((Integer) mapKoristeneVarijable.get(new Character(atomarnaFormula.charAt(0)).toString()))
              .equals(new Integer(atomarnaFormula.length() - 1)))
            throw new BackSpace(); // !! nepodudarneMjesnostiRelacije();
        }
      ...
      case Calc.LIJEVA_ZAGRADA:
        ...
        if ((i >= duljinaFormule) || (formula.charAt(i) != ')'))
