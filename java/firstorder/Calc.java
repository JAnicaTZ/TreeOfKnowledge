...
/**
 * FIRST-ORDER LOGIC CALCULATOR Part of the TreeOfKnowledge.eu project.
 *
 * <p>🕯 Dedicated to every child who dies from starvation — 1 every 10 seconds, around 10,000 each
 * day.
 *
 * <p>BEST CORE of AI Author: JAnica Tesla Zrinski Domain: https://TreeOfKnowledge.eu Years:
 * 2002–2025
 *
 * <p>All rights reserved.
 *
 * <p>This source code is the intellectual property of JAnica Tesla Zrinski (TreeOfKnowledge.eu).
 *
 ...
 *
 * <p>This class provides the interactive Swing UI used on TreeOfKnowledge.eu: it lets the user
 * build a formula (quantifiers, variables, predicates and connectives), shows the typed expression,
 * and triggers parsing + the engine pipeline.
 *
 * <p>Core responsibilities:
 *
 * <ul>
 *   <li>UI construction: buttons for ∀/∃, variables, predicates, connectives, parentheses, etc.
 *   <li>Input handling: appending symbols, backspace/clear, and loading prepared examples.
 *   <li>Parsing: delegates the current input string to {@link Parser#parsiraj(String)}.
 *   <li>Execution pipeline: runs the recursive engine (normalization / main test) and coordinates
 *       solution search / visualization (via {@link PronalazenjeRjesenja}).
 * </ul>
 *
...
public class Calc extends JFrame implements ActionListener {
  ...
  // @Zašto propCommon zove firstorder.Calc?
  ...
  public Calc() {
    ...
    // JLabel testFormule = new JLabel( "Ready-made examples of FOL formulas: ",
    // JLabel.CENTER);
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (action.equals(clear)) {
      ...
      if (nosacOd2ElementaPanel.getComponentCount() > 2)
        nosacOd2ElementaPanel.remove(2);
      if (nosacOd2ElementaPanel.getComponentCount() > 1)
        nosacOd2ElementaPanel.remove(1);
      nosacOd2ElementaPanel.add(new JButton("glavni test"));
      nosacOd2ElementaPanel.add(new JButton("za  NOSAC OD 2 elem."));
      display.setText(" ");
    }
    ...
    if (action.equals(enter)) {
      ...
      if (display.getText().length() > 1) {
        if (Calc.nosacOd2ElementaPanel.getComponentCount() > 2)
          Calc.nosacOd2ElementaPanel.remove(2);
        if (Calc.nosacOd2ElementaPanel.getComponentCount() > 1)
          Calc.nosacOd2ElementaPanel.remove(1);
        try {
          ...
          if (display.getText().length() > 1 && Parser.i < Parser.duljinaFormule)
            display.setText(" " + Parser.formula.substring(0, Parser.i));
        }
      }
    }
  ...
  public static void main(String args[]) throws Exception {
    ...
    // --------------------------------------------------------------
    System.out.println(
        "⚠️WARNING: Do NOT be wise in your OWN eyes: fear the Lord and shun evil. ~Proverbs 3:7");
    // -------------------------------------------------------------------
