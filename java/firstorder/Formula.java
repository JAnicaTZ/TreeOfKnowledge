 ...
 *
 * <p>This hierarchy represents formulas as an Abstract Syntax Tree (AST), where each node is
 * either an atomic predicate (leaf), a negation (unary node), or a binary connective
 * (conjunction/disjunction).
 *
 * <p>Core responsibilities defined at this level:
 * <ul>
 *   <li>{@link #toString()} – operator / node label used for rendering and debugging,</li>
 *   <li>{@link #prikazFormule()} – builds a Swing tree node for visualization,</li>
 *   <li>{@link #glavniTestLogikePrvogReda()} – runs the main recursive transformation/test pipeline
 *       (normalization + preparation for evaluation).</li>
 * </ul>
 *
 * <p>The engine is designed around a recursive “tree-first” workflow: transformations and tests
 * are performed by descending into the AST, transforming subtrees, and returning an updated tree.
 */
