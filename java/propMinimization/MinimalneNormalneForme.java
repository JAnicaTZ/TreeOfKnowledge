...
public class MinimalneNormalneForme{
  ...
  public static void minimalneNormalneForme(Formula korijenStabla){
    ...
    OsvjetljivanjeRjesenja.osvijetliRjesenja( StabloFormule.koristeneVarijable, disjunktivnaNF, Calc.interpretacijePanel);
	...
	public static String ispisDisjunkta( boolean disjunktivna, List disjunkt){// ubaceno!!  SORTIRANJE
		String izlaz = "( ";
		int i = 0;
		AtomarnaFormula propozicionalnaVarijabla;
		while (izlaz.equals("( ")){
			propozicionalnaVarijabla = new AtomarnaFormula(((Character) StabloFormule.koristeneVarijable.get(i++)).charValue());
		...
		for ( int j = i; j < StabloFormule.koristeneVarijable.size(); j++){
			propozicionalnaVarijabla = new AtomarnaFormula(((Character) StabloFormule.koristeneVarijable.get(j)).charValue());
			if (disjunkt.contains(propozicionalnaVarijabla)){
				if (disjunktivna) izlaz += (new Character(Calc.AND_CHAR)).toString() + propozicionalnaVarijabla.toString();
				else izlaz += (new Character(Calc.OR_CHAR)).toString() + propozicionalnaVarijabla.suprotnaFormula().toString();
			}
			propozicionalnaVarijabla = propozicionalnaVarijabla.suprotnaFormula();
			if (disjunkt.contains(propozicionalnaVarijabla)){
				if (disjunktivna) izlaz += (new Character(Calc.AND_CHAR)).toString() + propozicionalnaVarijabla.toString();
