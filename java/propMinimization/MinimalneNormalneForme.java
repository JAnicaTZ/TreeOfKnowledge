...
public class MinimalneNormalneForme {
  ...
  public static void minimalneNormalneForme(Formula korijenStabla) {
    ...
    if (!disjunktivnaNF.isEmpty()) {
      ...
      if (!primeImplicants.contains(new String("tautology;)!)"))) {
        List ireducibilneDNF = IrreducibleDNF.ireducibilneDNF(primeImplicants);
        // System.out.println("\n\nIRREDUCIBLE DISJUNCTIVE NORMAL FORMS (DNF):\n\n" +
        // ispisDNFormi( true, ireducibilneDNF));
        List konjunktivnaNF = ((NormalFormFormula) new Negation((Formula) korijenStabla.clone()).eliminiramNegacije())
            .disjunktivnojFormi();
        List ireducibilneKNF = IrreducibleDNF.ireducibilneDNF(PrimeImplicants.primeImplicants(konjunktivnaNF));
        ...
        String minimalneNormalneForme = "DISJUNCTIVE  Normal Forms (DNFs) with " + minimalanBrojLiterala
            + " literals:\n";
        ...
        if (minimalneNormalneForme.length() > 0)
          minimalneNormalneForme += "\n\n";
        ...
        if (minimalnaSlozenost > minimalnaSlozenostKNF)
          minimalnaSlozenost = minimalnaSlozenostKNF;
        ...
        for (int i = 0; i < ireducibilneKNF.size(); i++) {
          ...
          if (slozenostDNForme == minimalnaSlozenost) {
            ...
            if (minimalneNormalneForme
                ...
                .indexOf(minimalnaKonjunktivnaNormalnaForma) < 0)
        ...
        Calc.minimalneNormalneForme.append("\t              TAUTOLOGY\n");
    } else
      Calc.minimalneNormalneForme.append("\t          ANTItautology\n");
  ...
  public static String ispisDNForme(boolean disjunktivna, List disjunktivnaNF) {
    ...
    for (int i = 1; i < disjunktivnaNF.size(); i++) {
      if (disjunktivna)
        izlaz += " " + (new Character(UIStrings.OR_CHAR)).toString() + " ";
      else
        izlaz += " " + (new Character(UIStrings.AND_CHAR)).toString() + " ";
  ...
  public static String ispisDisjunkta(
    ...
    String izlaz = "( ";
    int i = 0;
    AtomicFormula propozicionalnaVarijabla;
    while (izlaz.equals("( ")) {
      propozicionalnaVarijabla = new AtomicFormula(
          ((Character) FormulaTreeParser.koristeneVarijable.get(i++)).charValue());
      if (disjunkt.contains(propozicionalnaVarijabla)) {
        if (disjunktivna)
          izlaz += propozicionalnaVarijabla.toString();
        else
          izlaz += propozicionalnaVarijabla.suprotnaFormula().toString();
      }
    ...
    for (int j = i; j < FormulaTreeParser.koristeneVarijable.size(); j++) {
      propozicionalnaVarijabla = new AtomicFormula(
          ((Character) FormulaTreeParser.koristeneVarijable.get(j)).charValue());
      if (disjunkt.contains(propozicionalnaVarijabla)) {
        if (disjunktivna)
          izlaz += (new Character(UIStrings.AND_CHAR)).toString() + propozicionalnaVarijabla.toString();
        else
          izlaz += (new Character(UIStrings.OR_CHAR)).toString()
  ...
  public static int minimalnaSlozenost(boolean brojiDisjunkte, List listaDNFormi) {
    ...
    for (int i = 1; i < listaDNFormi.size(); i++) {
      ...
      if (slozenost < minimalnaSlozenost)
        minimalnaSlozenost = slozenost;
    }
  ...
  public static int slozenostDNForme(boolean brojiDisjunkte, List disjunktivnaNF) {
    ...
    if (brojiDisjunkte)
      slozenostDNForme = disjunktivnaNF.size();
