...
public class OsvjetljivanjeRjesenja{
  ...
  public static void osvijetliRjesenja( List koristeneVarijable, List disjunktivnaForma, JPanel panel){
    ...
    for ( int i = 0; i < semantickaTablica.length; i++){
      ...
      while (++k <= disjunktivnaForma.size())
				if (OsvjetljivanjeRjesenja.zadovoljavaInterpretaciju( semantickaTablica[i], (List) disjunktivnaForma.get(k-1))){
